...
from perception import VideoPerception
from features import TrackFeatureExtractor
    ...
    def __init__(
        ...
        Args:
            ...
            recall_engine: 召回引擎。如果为 None，使用默认的 RecallEngine
                          支持依赖注入，方便测试和替换召回策略
            vlm_client: VLM客户端。如果为 None，使用默认的 QwenVLMClient
                       支持依赖注入，方便测试和使用不同的VLM模型
        ...
        self.vlm_client = vlm_client or QwenVLMClient(self.config)
